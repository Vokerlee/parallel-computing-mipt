...
int main(int argc, char* argv[])
{
    ...
    if (comm_rank > 0)
    {
        ...
        snprintf(format_str, format_str_size, "%%.%dFf", n_decimal_places);
    ...
    else if (comm_rank == 0 && comm_size > 1)
    {
        ...
        snprintf(format_str, format_str_size, "%%.%dFf\n", n_decimal_places);
...
/* 
    ln(N!) = N * ln(N) - N
    So as N! = 10^k, where k is degree of ten, k = N * log10(k) - N =>
    N = k / (log10(k) - 1), then add some additional value to 
    calculate amount of series with a margin 
*/
long int estimate_n_series_members(long int decimal_precision)
{
    if (decimal_precision <= 20) // take with a margin for paralleling
        return 24;
    else
    {
        long int decimal_precision_log10 = log10(decimal_precision);
        double n = (double) decimal_precision *
            (1. + 0.007 * decimal_precision_log10) / (decimal_precision_log10 - 1);
